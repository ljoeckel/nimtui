...
proc YesNoDialog*(title: string, content: string, yes: proc(), no: proc() = nil) =
    proc onYes(v: Widget): EventHandler = 
        result = EventHandler(view: v)
        result.onAction = proc(v: Widget) =
            yes()
            removeView(v.id)

    proc onNo(v: Widget): EventHandler =
        result = EventHandler(view: v)
        result.onAction = proc(v: Widget) =
            if no != nil: no()
            removeView(v.id)
    ...
    if findView("dlg").isNil:
        ...
        dlg.add(Button(id:"yes", name:"Yes", frame:1, textstyle:FAINT, align:BOT_CENTER, handler:onYes(dlg)))
        dlg.add(Button(id:"no", name:"No", frame:1, align:BOT_CENTER, handler:onNo(dlg)))
        addView(dlg)
        setFocus(dlg, "no")